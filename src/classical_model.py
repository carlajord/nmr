...
class XGBoostModel:
    ...
    def __init__(
        ...
        self.feature_extractor = FeatureExtractor(use_pca=use_pca)
...
def plot_xgboost_prediction_analysis(
    ...
    targets: np.ndarray,
    save_path: str = os.path.join(RESULTS_PATH, 'xgboost_prediction_analysis.png')
):
    """
    Create comprehensive prediction analysis plots for XGBoost model.

    Args:
        predictions: Predictions from XGBoost model
...
def optimize_xgboost_hyperparameters(
    ...
    def objective(trial):
        ...
        # Add min_child_weight and gamma to the underlying XGBoost model
...
def _plot_optimization_history(
    study,
    save_path: str = os.path.join(RESULTS_PATH, 'optuna_optimization_history.png')
):
    """
...
def run():
    ...
    Returns:
        Tuple of (model, predictions, metrics, test_data)
    """
    ...
    xgb_model = XGBoostModel(
        ...
        feature_method='raw',#'statistical'
        use_pca=False
    )
    ...
    return xgb_model, predictions, metrics, (X_test, y_test)
...
def run_with_optimization(
    ...
    print("="*80)
    mse, mae, r2, predictions = evaluate_xgboost_model(best_model, X_test, y_test)
    ...
    metrics = {
        ...
        'n_trials': n_trials,
        'n_features': best_model.feature_extractor.extract_statistical_features(X_train[:1]).shape[1] if use_feature_engineering else X_train.shape[1] * X_train.shape[2] * X_train.shape[3]
    }
    ...
    # Generate prediction analysis plots
    plot_xgboost_prediction_analysis(predictions, y_test,
                                     save_path=os.path.join(RESULTS_PATH, 'xgboost_optimized_prediction_analysis.png'))
